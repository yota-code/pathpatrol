...
class Piece(Polygon) :
	def __init__(self, x_lst=None, y_lst=None) :
		Polygon.__init__(self, x_lst, y_lst)

		self.m_arr = np.array([0,] * len(self), dtype=np.int8)
		self.cut()
		self.sort()
	...
	def sort(self) :
		self.convex = Polygon(
			self.x_arr[np.where(self.m_arr== 0)],
			self.y_arr[np.where(self.m_arr== 0)]
		)
		self.cavity = dict()
		for i in range(0, len(self)) :
			...
			if m > 0 :
				b = i + 2
				self.cavity[(a, b)] = Polygon(* self[a:b])
		# print(self.cavity)
