...
class Route() :
	def __init__(self, layer) :
		self.layer = layer
		self.carte = [piece.shape for piece in layer.g_lst] # on prend les polygones originaux
		self.route = RouteMap()

	def prune_layer(self, A, B) :
		pass
	...
	def compute(self, from_point, to_point) :
		...
		for z in range(100) :
			...
			plt.grid()
			plt.axis("equal")
			plt.savefig(f"img/{z:04d}.a.png")
			plt.close()

			for sgt, status in self.route :
				if status is None : # the collision status is unknown, let's investigate
					A, B = sgt

					# le status des collisions est inconnu, il faut fouiller
					p, i_lst = self.first_collision(A.val, B.val)
					...
					plt.plot(
						...
						[ay*(1 - t) + by*t for t, i, w in i_lst], 'o', color="tab:orange"
					)
					plt.grid()
					plt.axis("equal")
					plt.savefig(f"img/{z:04d}.b.png")
					# plt.close()
					plt.show()

					### les contournements devrait faire appel à une fonction
	...
	def first_collision(self, A, B) :
		...
		"""
		for p, p_gon in enumerate(self.carte) :
			if not (p_gon.is_inside_box(A) or p_gon.is_inside_box(B) or p_gon.do_intersect_box(A, B)) :
				# the segment doesn't even pass through the polygon box, continue
				continue
			i_lst = p_gon.scan_intersection(A, B)
			if i_lst :
				return p, i_lst
		return None, list()
